...
> Claude Code 从安装到上手的完整教程。安装前有任何疑问？先问 ChatGPT 等 AI 对话工具；安装好之后，直接在 Claude Code 里问它就好。
...
Claude Code 的核心交互方式就是自然语言——你说人话，它干活。
...
**然后用自然语言描述需求：**
- "帮我把这个文件夹里的 PDF 整理成一份表格"
- "读一下这个文件，告诉我核心观点"
- "帮我润色这段文字"

**在文渊中使用时**，还可以用 `/` 命令触发预设的学术工作流（如 `/read` 精读文献、`/brainstorm` 头脑风暴）。详见 [快速上手教程](快速上手教程.md)。
